...
import { Search, Globe, Package, MapPin, Calendar, Ship, ArrowRight, AlertCircle, Loader2 } from 'lucide-react';
import { useLazyGetShipmentByTrackingQuery, useGetVesselByIdQuery } from '../features/api/apiSlice';
...
export const TrackingPage = () => {
  ...
  const [triggerSearch, { data: shipment, isLoading: loadingShipment, isError }] = useLazyGetShipmentByTrackingQuery();
    ...
    <div className="min-h-screen bg-slate-950 flex flex-col p-4 relative overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-4xl mx-auto mt-10 text-center space-y-8">
        <div className="flex justify-center items-center gap-3">
            ...
            <h1 className="text-4xl font-bold text-white tracking-tight">Neptune Tracking</h1>
        </div>
        ...
        <div className="bg-slate-900/80 backdrop-blur-md p-6 rounded-2xl border border-slate-700 shadow-2xl mx-auto max-w-xl">
            <form onSubmit={handleSearch} className="relative">
                ...
                <button type="submit" className="absolute right-2 top-2 bottom-2 bg-blue-600 hover:bg-blue-500 text-white px-6 rounded-md transition-colors font-bold">
        ...
        <div className="relative z-10 w-full max-w-6xl mx-auto mt-8 grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="bg-slate-900/90 backdrop-blur border border-slate-800 rounded-xl p-6 shadow-xl h-fit">
                <div className="flex items-center gap-3 mb-6 border-b border-slate-800 pb-4">
                    ...
                    <div>
                        <h2 className="text-xl font-bold text-white">{shipment.tracking_number}</h2>
                        <p className="text-slate-400 text-sm">Customer: {shipment.customer_name}</p>
                    </div>
                </div>
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-slate-950 rounded-lg p-4 space-y-3">
                        <div className="flex justify-between text-sm">
                            ...
                            <span className="text-green-400 font-bold">{shipment.status}</span>
                        </div>
                        <div className="flex justify-between text-sm">
                        ...
                        <div className="flex justify-between text-sm">
                            ...
                            <span className="text-white font-mono">{shipment.container_number || 'N/A'}</span>
                        </div>
                    </div>
                </div>
            </div>
                ...
                {vessel && (
                    <div className="bg-blue-900/20 border border-blue-500/30 rounded-xl p-4 flex items-center justify-between text-blue-100">
                        <div className="flex items-center gap-3">
                            <Ship />
                            <span>Carried by <b>{vessel.name}</b> ({vessel.imo_number})</span>
                        </div>
